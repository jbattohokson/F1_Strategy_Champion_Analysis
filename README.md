...
```
F1_Strategy_Champion_Analysis/
├── Python Code/
│   └── F1_Strategy_Champion_V2.py   ← the only file needed
├── f1_cache/                         ← gitignored (auto-populated on run)
├── raw_data/                         ← gitignored (generated by script)
├── tableau_export/                   ← gitignored (generated by script)
├── charts/                           ← gitignored (generated by script)
├── archive/
│   ├── F1_Strategy_Champion_V1.py
│   └── F1_Strategy_Champion_V3.py
└── README.md
```
...
# 2. Run the pipeline — fetches and caches all F1 data automatically
python "Python Code/F1_Strategy_Champion_V2.py"
```

Running `F1_Strategy_Champion_V2.py` fetches and caches everything automatically. On first run, FastF1 downloads session data to `f1_cache/` (~20 min). Subsequent runs load from cache and are much faster. CSVs are exported to `tableau_export/` and charts to `charts/`.
